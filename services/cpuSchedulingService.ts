...
export const calculateSchedule = (
  ...
  let executedProcessIds = new Set<number>(); 
  ...
  switch (algorithm) {
    ...
    case Algorithm.SRTF: 
      ...
      let sjfProcesses = [...processes].map(p => ({...p, remainingTime: p.burstTime, lastExecutionTime: p.arrivalTime})); 
      ...
      while(completedProcesses.length < processes.length) {
        ...
        if(!isPreemptiveSJF) readyQueue.shift(); 
        
        if(!executedProcessIds.has(currentProcess.id)){ 
    ...
    case Algorithm.ROUND_ROBIN:
      ...
      while (completedProcesses.length < processes.length) {
        ...
        let currentProcess = rrQueue.shift()!; 
        queueEvents.push({ type: 'dequeue', processId: currentProcess.id, processName: currentProcess.name, time: currentTime });
