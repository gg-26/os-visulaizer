    ...
    <div className="p-6 bg-white dark:bg-slate-800 rounded-lg shadow-lg space-y-6">
      ...
      <div className="space-y-3 p-4 border border-slate-200 dark:border-slate-700 rounded-md">
        ...
        {currentAlgoDetails?.hasTimeQuantum && (
          <div className="flex items-center justify-between">
            <label htmlFor="time-quantum" className="text-sm font-medium text-slate-700 dark:text-slate-300">Time Quantum:</label>
            <Input
              type="number"
              id="time-quantum"
              value={timeQuantum}
        ...
        {currentAlgoDetails?.hasPriority && (
           <div className="flex items-center justify-between">
            <label htmlFor="priority-order" className="text-sm font-medium text-slate-700 dark:text-slate-300">Priority Meaning:</label>
            <Select
              id="priority-order"
              value={priorityOrder}
         ...
         {!(currentAlgoDetails?.hasTimeQuantum || (currentAlgoDetails?.id === Algorithm.PRIORITY && currentAlgoDetails?.canBePreemptive) || (currentAlgoDetails?.id === Algorithm.SJF && currentAlgoDetails?.supportsSjfPreemption)) && (
      ...
      <div>
        ...
        <div className="space-y-1 max-h-96 overflow-y-auto p-1 border border-slate-200 dark:border-slate-700 rounded-md">
           <div 
            className={`grid ${showPriorityColumn ? 'grid-cols-[auto_1fr_1fr_1fr_auto]' : 'grid-cols-[auto_1fr_1fr_auto]'} gap-2 p-2 bg-slate-100 dark:bg-slate-700 rounded-t-md sticky top-0 z-10 text-xs sm:text-sm`}
            style={{gridTemplateColumns: showPriorityColumn ? 'auto minmax(60px,1fr) minmax(60px,1fr) minmax(60px,1fr) auto' : 'auto minmax(60px,1fr) minmax(60px,1fr) auto'}}
            >
            <span className="font-semibold">ID</span>
            {showPriorityColumn && <span className="font-semibold">Priority</span>}
